...
CREATE TABLE FitServe.health_restrictions ( -- Andriy
...
CREATE TABLE FitServe.proteins( -- Andriy
...
CREATE TABLE FitServe.studios ( -- lily
  studio_id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, -- Oleksii_with_ChatGPT_Sol_5.6
...
CREATE TABLE FitServe.pack_subscription ( -- Oleksii
  ...
  CONSTRAINT chk_pack_subscription_start_limit
   CHECK (start_date <= CURRENT_DATE + INTERVAL '1 year'), -- lily-CHECK

  -- Oleksii-CHECK: chk_pack_subscription_start_not_past was removed because
  -- a CURRENT_DATE-based CHECK becomes unsuitable for historical subscriptions
  -- and can prevent restoring previously valid data. -- Oleksii_with_ChatGPT_Sol_5.6
...
CREATE TABLE FitServe.payment ( -- Oleksii
  ...
  CONSTRAINT chk_payment_amount_not_exceed_price
   CHECK (amount_paid <= price), -- Oleksii-CHECK
...
ALTER TABLE FitServe.health_restrictions
    DROP CONSTRAINT IF EXISTS health_restrictions_athlete_id_fkey,
    ADD CONSTRAINT fk_health_restrictions_athlete
        FOREIGN KEY (athlete_id)
        REFERENCES FitServe.athletes (athlete_id)
        ON UPDATE CASCADE
        ON DELETE CASCADE;
...
ALTER TABLE FitServe.proteins
    DROP CONSTRAINT IF EXISTS proteins_nutrition_id_fkey,
...
ALTER TABLE FitServe.payment
    ...
    ADD CONSTRAINT fk_payment_pack_subscription
        ...
        ON UPDATE CASCADE
        ON DELETE CASCADE; -- Oleksii_with_ChatGPT_Sol_5.6
...
CREATE INDEX IF NOT EXISTS idx_health_restrictions_athlete_id
    ON FitServe.health_restrictions (athlete_id);
